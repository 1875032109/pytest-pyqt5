...
def test_update_bmc_inband_preserve():
    # 根据产品制造商选择配置
    script_preserve_downgrade, script_preserve_upgrade = get_update_bmc_inband_script(
    ...
    # SSH进入OS,进入到BMC升降级文件夹,执行升降级脚本进行降级（保留配置）
    logging.info("Inband Update BMC Begin")
    inband_update_log = ssh_execute_command(
        os_ip,
        os_username,
        os_password,
        f"cd {inband_update_file_path} && {script_preserve_downgrade}",
    )
    ...
    assume(
        list_diff(user_info_before, user_info_middle, "user")
    ), "保留配置降级后和初始user信息不一致"
    assume(
        dict_diff(bmc_network_before, bmc_network_middle, "bmc_network")
    ), "保留配置降级后和初始network信息不一致"
    assume(
        list_diff(sdr_info_before, sdr_info_middle, "sdr elist")
    ), "保留配置降级后和初始sdr信息不一致"
    assume(
        list_diff(sensor_info_before, sensor_info_middle, "sensor list")
    ), "保留配置降级后和初始sensor信息不一致"

    # 获取sel并检查是否存在告警sel
    Sel_Check()

    # SSH进入OS,进入到BMC升降级文件夹，执行升降级脚本进行升级（保留配置）
    logging.info("Inband Update BMC Begin")
    inband_update_log = ssh_execute_command(
        os_ip,
        os_username,
        os_password,
        f"cd {inband_update_file_path} && {script_preserve_upgrade}",
    )
    assert inband_update_log, "Inband Update Failed"
    if machine_model == "G220-BA-B":
        time.sleep(60)
