...
def test_update_bmc_inband_default():
    # 根据产品制造商选择配置
    ...
    # SSH进入OS,进入到BMC升降级文件夹,执行升降级脚本进行降级（不保留配置）
    logging.info("Inband Update BMC Begin")
    inband_update_log = ssh_execute_command(
        os_ip,
        os_username,
        os_password,
        f"cd {inband_update_file_path} && {script_default_downgrade}",
    )
    ...
    # SSH进入OS,进入到BMC升降级文件夹，执行升降级脚本进行升级（不保留配置）
    logging.info("Inband Update BMC Begin")
    inband_update_log = ssh_execute_command(
        os_ip,
        os_username,
        os_password,
        f"cd {inband_update_file_path} && {script_default_upgrade}",
    )
    assert inband_update_log != False, "Inband Update Failed"
    if machine_model == "G220-BA-B":
        pass
    else:
        time.sleep(360)
